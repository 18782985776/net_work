...
/**
 * Created by Marno on 2017/1/10/15:34
 * Function：
 * Desc：
 */
public class UpdateDataDelegate {
    ...
    public void initLoad(BaseQuickAdapter.RequestLoadMoreListener listener,
                         BaseQuickAdapter adapter, RecyclerView.LayoutManager manager) {
        getRecyclerView(mRootView);

        if (null != manager && null != mRecyclerView)
            mRecyclerView.setLayoutManager(manager);

        setAdapter(listener, adapter);
    }
...
//        }
    ...
    private void setAdapter(BaseQuickAdapter.RequestLoadMoreListener listener, BaseQuickAdapter adapter) {
        if (null != adapter && null != mRecyclerView) {
            adapter.setOnLoadMoreListener(listener,mRecyclerView);
