...
public class BreedingSchemeFragment extends BaseFragment {
    ...
    public static BreedingSchemeFragment newInstance(int seeId, int duration, int num, boolean isFirst, ArrayList schemeList) {
        Bundle args = new Bundle();
        //保存过默认方案，再次打开需要传入方案数据，使其恢复
        if (!isFirst) {
            args.putParcelableArrayList(Constant.SCHEME, schemeList);
        }
        args.putInt(Constant.PRODUCT_ID, seeId);
        args.putInt(Constant.DURATION, duration);
        args.putInt(Constant.NUMBER, num);
        BreedingSchemeFragment recommendSchemeFragment = new BreedingSchemeFragment();
    ...
    protected void initView(Bundle bundle, View view) {
        ...
        mAdapter = new RecomSchemeAdapter(getActivity(), mList, number, false);
    ...
    private void initIntent() {
        ...
        if (args != null) {
            mProductId = args.getInt(Constant.PRODUCT_ID, -1);
            mDuration = args.getInt(Constant.DURATION, 1);
    ...
    private void requestData() {
        loading();
        FarmRepository.getIns().getDefaultScheme(mProductId).subscribe(new DefaultSubscriber<RecommendSchemeEntity>(this, false) {
            @Override
            public void _onNext(RecommendSchemeEntity recommendSchemeEntity) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (entity.ctype == 1 || entity.ctype == 6) {
